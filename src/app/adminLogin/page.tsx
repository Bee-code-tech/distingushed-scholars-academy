...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Cast response payload to allow dynamic lookup across backend variants
      const resPayload = res as Record<string, any>
      const token =
        resPayload?.token || resPayload?.accessToken || resPayload?.data?.token
      ...
      if (token) {
        ...
        throw new Error(
          'Authentication failed. No auth token returned by backend.',
        )
      }
    } catch (err: any) {
      ...
      <motion.div layout className='w-full max-w-md z-10'>
        <div className='text-center mb-8'>
          <motion.div
            animate={isSuccess ? { scale: 1.1, rotate: 360 } : { scale: 1 }}
            transition={{ type: 'spring', stiffness: 200 }}
            className={`inline-flex p-4 rounded-3xl bg-slate-900 border ${
              isSuccess
                ? 'border-emerald-500/50 shadow-emerald-500/20'
                : 'border-slate-800 shadow-blue-500/10'
            } shadow-2xl mb-4 transition-all duration-500`}
          >
            {isSuccess ? (
              <CheckCircle2 size={40} className='text-emerald-500' />
            ) : (
        ...
        >
          <form onSubmit={handleLogin} className='space-y-5'>
            ...
            <button
              ...
              disabled={loading || isSuccess}
              className={`w-full ${
                isSuccess ? 'bg-emerald-600' : 'bg-blue-600 hover:bg-blue-500'
              } disabled:bg-slate-800 text-white rounded-2xl py-4 text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-600/20 active:scale-95`}
            >
              {loading ? (
                <Loader2 className='animate-spin' size={18} />
              ) : isSuccess ? (
